...
一个纯前端的飞书多维表格边栏插件：选择附件列和记录，使用同一行的一列或多列内容组合命名附件，自定义组合顺序与分隔符，并按另一列的内容分类到文件夹，最终下载 ZIP。

## 当前状态

- 已部署到 [正式插件地址](https://shandianchengzi.github.io/feishu-file-export/)，可填写到飞书边栏插件的托管地址中。插件名称可填写“附件批量导出”。
- 本地测试已扩展到 17 项。GitHub Actions 会对每次代码提交运行全部测试、TypeScript 检查与生产构建，通过后发布网页。
...
- [首次成功部署的 Actions 记录](https://github.com/shandianchengzi/feishu-file-export/actions/runs/34690208464)：`build` 和 `deploy-project-pages` 均成功，部署输出确认正式地址为 `https://shandianchengzi.github.io/feishu-file-export/`。后续更新见 [Actions 列表](https://github.com/shandianchengzi/feishu-file-export/actions)。

## 功能

1. 选择数据表与视图，读取当前视图、整张表或交互式选择的记录。当前视图模式遵循该视图的筛选，使用分页 API，不限于前 200 条。
2. 勾选一个或多个附件列。选择记录与附件列的交集即确定需要读取的附件单元格，预览后还能逐个勾选附件。
3. 多选命名列，按勾选顺序组合附件所在行的单元格内容，可用上下按钮调整顺序。分隔符默认 `_`，可以自定义为 `-`、空格等，留空直接拼接。支持“组合内容 + 原扩展名”或“组合内容 + 分隔符 + 原文件名”。空单元格自动跳过，全部为空或未选择命名列时保留原文件名。
...
多列示例：依次选择“学号”“姓名”，分隔符填 `-`，分类列选“学院”，可导出为 `计算机学院/2026001-张三.pdf`。使用保留原文件名模式时，导出为 `计算机学院/2026001-张三-申请材料.pdf`。分隔符中的斜杠等文件名非法字符会替换为下划线，不会创建额外目录。
...
飞书插件上架表单要求公开仓库提供 `dist/index.html`。仓库中的 `dist/` 是提交审核的编译快照；Actions 继续自动构建、上传产物并更新 Pages，不自动回写源码分支。每次申请插件市场版本更新时，需要用对应提交的 Actions 编译产物同步此目录。

在普通浏览器中打开页面会显示安装说明。访问 `?demo=1` 可用内置小文件验证预览、分类、重名及 ZIP 下载；示例模式与飞书数据完全分离。真实插件 URL 不应添加 `?demo=1`。

测试覆盖多列命名顺序、自定义/空分隔符、空值跳过、分页超过 200 条、分页异常、命名/分类碰撞、ZIP 内的组合路径与原始字节、失败清单、过期链接重试、流式大小上限及取消。
